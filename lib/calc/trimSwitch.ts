 ...
 * (except eaves/gables trim selection, which is brand-independent) resets and
 * re-derives from scratch.
 */
export function trimConfigForBrand(brand: Brand, current?: TrimConfig): TrimConfig {
  // Eaves/gables trim selection and step flashing are situational (roofline shape), not
  // brand-driven — preserve them across a material change instead of resetting.
  const eavesTrim = current?.eavesTrim ?? defaultTrimConfig().eavesTrim;
  const gablesTrim = current?.gablesTrim ?? defaultTrimConfig().gablesTrim;
